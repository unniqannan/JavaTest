 ...
 * @author AnujKumar
 */
public class BranchPage extends Services {

    static Logger logger = Logger.getLogger(BranchPage.class.getName());
    ...
    public int createAndCancel(String name, String code) {
        clickOnBtn(ActionsEnum.Create.name());
        fillCreateOrEditForm(name, code);
        clickOnBtn(ActionsEnum.Cancel.name());
        waitForElementInvisibility(xpathBtn.replace("*btn*", ActionsEnum.Cancel.name()));
        return getMaxId();
    }
